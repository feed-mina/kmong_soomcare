...
<%
	...
	//날짜 세팅
	Date searchStartMonth = GF.stringToDateForMonth(GF.getString(request.getParameter("MONTH"))+"01");
	String Day_GreaterThan = GF.dateToDateStringHyphen(searchStartMonth);
	
	Date searchEndMonth = GF.stringToDateForMonth(GF.increaseMonth( GF.getString(request.getParameter("MONTH")))+"01");
	...
	if(arrUser.get(0).GENDER == 1){
		gender = "남";
	}else if(arrUser.get(0).GENDER == 2){
		gender = "여";
	}else{
		gender = "-";
	}
	if(arrUser.get(0).DIAGNOSIS_FLAG != 1){
		definiteDiagnosis = "의심";
	}
	...
	String startDt = request.getParameter("MONTH")+"01";
	String lastDt = request.getParameter("MONTH")+lastDay;
	...
	for(int i=0;i<arrMedicineHistoryNew.size();i++){
		if(df.parse(arrMedicineHistoryNew.get(i).START_DT).compareTo(df.parse(GF.getString(request.getParameter("MONTH"))+"01")) < 0){
			if(df.parse(arrMedicineHistoryNew.get(i).END_DT).compareTo(df.parse(GF.getString(request.getParameter("MONTH"))+lastDay)) > 0){
	...
	int isSickDayCnt = 0, nightSickCnt = 0, coughCnt = 0, breathCnt = 0, roaringCnt = 0, chestCnt = 0, etcCnt = 0,phlegmCnt = 0,noSymptomCnt = 0, causeCnt=0;
	...
	int totalCoughCnt = 0, totalBreathCnt = 0, totalRoaringCnt = 0, totalChestCnt = 0, totalEtcCnt = 0, totalPhlegmCnt = 0, totalNoSymptomCnt = 0;
	
	for(int i=0;i<arrCauseSize;i++){
// 		System.out.println("REGISTER_DT: "  +  GF.stringToDate(arrFeed_CAUSE.get(i).REGISTER_DT));
		//증상없음 필터링 할 부분
		...
		if( GF.stringToDate(arrFeed_CAUSE.get(i).REGISTER_DT).getHours() >= 21 || GF.stringToDate(arrFeed_CAUSE.get(i).REGISTER_DT).getHours() < 6 ){
			...
			if( nightDateArr.contains((GF.stringToDate(arrFeed_CAUSE.get(i).REGISTER_DT)+""))==false ){
				if(arrFeed_CAUSE.get(i).CATEGORY != 40){
					String temp = GF.stringToDate(arrFeed_CAUSE.get(i).REGISTER_DT)+"";
	...
	int totalEmergencyMedi = 0;
	int totalMedi = 0;
	ArrayList<String> emergencyArr = new ArrayList<String>();
	int arrMediSize = arrFeed_MEDI.size();
	for(int i=0;i<arrMediSize;i++){
		totalMedi++;
		if(arrFeed_MEDI.get(i).EMERGENCY_FLAG == 2){
			if(emergencyArr.contains((GF.stringToDate(arrFeed_MEDI.get(i).REGISTER_DT).getDate()+""))==false){
				String temp = GF.stringToDate(arrFeed_MEDI.get(i).REGISTER_DT).getDate()+"";
				emergencyArr.add(temp);
				totalEmergencyMedi++;
			}
		}
	}
	...
	int actSum = 0, actAvg = 0, lastAct=0;
	String actRs = "No Data.";
	for(int i=0;i<arrFeed_ACT.size();i++){
		//actSum += arrFeed_ACT.get(i).ACT_SCORE;
		lastAct = arrFeed_ACT.get(i).ACT_SCORE;
	}
	if(arrFeed_ACT.size() != 0){
		//actAvg = actSum/arrFeed_ACT.size();
		actRs = ""+lastAct+"점/25점";
	}
	// PEF
	...
	for(int i=0;i<arrFeed_PEF.size();i++){
		if(minPEF > arrFeed_PEF.get(i).PEF_SCORE){
			minPEF = arrFeed_PEF.get(i).PEF_SCORE;
		}
		if(maxPEF < arrFeed_PEF.get(i).PEF_SCORE){
			maxPEF = arrFeed_PEF.get(i).PEF_SCORE;
		}
	}
	if(minPEF!=9999){
		minPEFRS = minPEF+" (L/m)";
	...
	<div class="background-wrap">
		<div class="page-1" style="display:block;float:left;width:1080px; height:2980px; position: relative;min-height: 1px;float: left; ">
			...
			<div class="card">
				...
				<div class="title-info-user-u1" style="position:absolute; margin-left: 424px; top:45px;color:#707070;font-size:16px;width:300px;">회원정보 : <%=arrUser.get(0).NAME %> (<%=gender %> / <%=GF.getAge(Integer.toString(arrUser.get(0).BIRTH)) %>세 )</div>
				<div class="title-info-user-i1" style="position:absolute; margin-left: 424px; top:75px;color:#707070;font-size:16px;width:300px;">천식여부 : <%=definiteDiagnosis %> (<%=arrUser.get(0).OUTBREAK_DT %>)</div>
				<div class="title-info-user-h1" style="position:absolute; margin-left: 732px; top:45px;color:#707070;font-size:16px;width:340px;overflow: hidden;">진료병원 : <%=hosName %></div>
				<div class="title-info-user-d1" style="position:absolute; margin-left: 732px; top:75px;color:#707070;font-size:16px;width:340px;overflow: hidden;">담당의사 : <%=docName %> <%=department %></div>
			</div>
			...
			for(int i=0;i<mediNoArr.size();i++){
				...
				%>
			<%}
			String mediForMonth = "No Data.";
			if(mediTotalPerCnt > 0 && mediTotalPerSum > 0){
				mediForMonth = String.format("%.2f", (mediTotalPerSum/mediTotalPerCnt)*100) + " %";
				if( (mediTotalPerSum/mediTotalPerCnt)*100 > 100){
					mediForMonth = "100%";
				}
			}
			%>
			...
			<div class="overall-medi2"style="position:absolute;left:374px;top:180px;width: 332px;height: 60px;border-radius: 4px;background-color: #dff5e0;">
				...
				<div class="overall-medi-val2" style="width: 30%;position:absolute;right:18px;top:18px;font-size:20px;text-align: right;font-weight: bold;"><%=totalEmergencyMedi %>일/<%=lastDay %>일</div>
			</div>
			<div class="overall-medi3"style="position:absolute;left:718px;top:180px;width: 332px;height: 60px;border-radius: 4px;background-color: #dff5e0;">
				...
				<div class="overall-medi-val1" style="width: 30%;position:absolute;right:18px;top:18px;font-size:20px;text-align: right;font-weight: bold;"><%=mediNoArr.size() %>개</div>
			</div>
			...
			<%
				...
				for(int i=0;i<mediNoArr.size();i++){
					...
					String tempFreq="";
					if(mediFrqArr.get(i)== 0 || mediFrqArr.get(i)== -1 ){
						tempFreq = "필요시 복용";
					}else{
						tempFreq = "하루 "+mediFrqArr.get(i)+"번";
					}
					...
					String goalStr = "";
					if(mediFrqArr.get(i) != -1){
						goalStr = ""+mediGoalCount.get(i)*mediFrqArr.get(i);
					}
					String mediPerStr = "필요시";
					double mediPer = 0;
					if(mediFrqArr.get(i)!=-1){
						mediPer = (emgMedi+norMedi)/(mediGoalCount.get(i)*mediFrqArr.get(i));
						...
						if(mediPer >= 1){
							...
							mediPerStr = ""+String.format("%.2f",mediPer*100) + "%";
						}
					}
					%>
					<div class="medi-card<%=i %>" style="position:absolute;left:<%=30+leftVal %>px;top:<%=250+topVal %>px;width: 332px;height: 144px;border-radius: 4px;border: solid 1px #acacac;background-color: #ffffff;">
						<div class="medi-card-name"style="position:absolute;left:12px;top:14px;width: 320px;font-size:18px;text-align: left; color: #343434;font-weight: bold;"><%=mediNameArr.get(i) %></div>
						...
						<div class="medi-card-t2"style="position:absolute;left:12px;top:86px;width: 160px;font-size:16px;text-align: left; color: #707070;">일반복용 : <%=norMedi %> 번</div>
						<div class="medi-card-t3"style="position:absolute;left:12px;top:110px;width: 160px;font-size:16px;text-align: left; color: #707070;">응급복용 : <%=emgMedi %> 번</div>
...
<%-- 					<div class="pef-pc"style="position:absolute;left:<%=199+118*wCnt%>px;top:<%=1842+301*hCnt%>px;font-size:16px;">최대</div> --%>
				...
				for(int i=0;i<arrFeed_PEF.size();i++){
					if( datePEFArr.contains((GF.stringToDate(arrFeed_PEF.get(i).REGISTER_DT).getDate()+""))==false ){
						String temp = GF.stringToDate(arrFeed_PEF.get(i).REGISTER_DT).getDate()+"";
						datePEFArr.add(temp);
						MaxPEFArr.add(arrFeed_PEF.get(i).PEF_SCORE);
						MinPEFArr.add(arrFeed_PEF.get(i).PEF_SCORE);
						inspiratorPEFMinArr.add(arrFeed_PEF.get(i).INSPIRATOR_FLAG);
						inspiratorPEFMaxArr.add(arrFeed_PEF.get(i).INSPIRATOR_FLAG);
					}else{//해당 날짜에 다른 데이터 존재
						int idx = datePEFArr.indexOf((GF.stringToDate(arrFeed_PEF.get(i).REGISTER_DT).getDate()+""));
						if(MaxPEFArr.get(idx) < arrFeed_PEF.get(i).PEF_SCORE){
							MaxPEFArr.set(idx , arrFeed_PEF.get(i).PEF_SCORE);
							inspiratorPEFMaxArr.set(idx, arrFeed_PEF.get(i).INSPIRATOR_FLAG);
						}
						if(MinPEFArr.get(idx) > arrFeed_PEF.get(i).PEF_SCORE){
							MinPEFArr.set(idx , arrFeed_PEF.get(i).PEF_SCORE);
							inspiratorPEFMinArr.set(idx, arrFeed_PEF.get(i).INSPIRATOR_FLAG);
						}
					}
				}
				double changeAvg = 0;
...
// 					}else{
						...
						if(inspiratorPEFMinArr.get(j)==2){
							temp1 = "report_pef_green.png";
						}else{
							temp1 = "report_pef_red.png";
						}
						if(inspiratorPEFMaxArr.get(j)==2){
...
// 							}else{
								...
								if(inspiratorPEFMinArr.get(i)==2){
									temp1 = "report_pef_green.png";
								}else{
									temp1 = "report_pef_red.png";
								}
								if(inspiratorPEFMaxArr.get(i)==2){
									temp2 = "report_pef_green.png";
								}else{
									temp2 = "report_pef_red.png";
								}
...
// 					}
			...
			<div class="card">
				...
				<div class="history-chart-mobile-bar-div" style="position:relative;border-radius: 25px;border-collapse: separate; z-index: 1;height:32px;overflow:hidden;margin-top:20px;margin-left:4%;width:92%;display:none;"><canvas class="history-chart-mobile-bar" id="chartJSContainerMobileBar" style="width:104%;height:74px;margin-top: -14px; margin-bottom: -7px;display:none;"></canvas></div>
				...
				<div class="cause-legend-val" style="position:absolute;display:inline-block;left:113px;top:1067px;font-size:20px;color: #364064;font-weight: bold;"><%=totalCoughCnt %>번</div>
				...
				<div class="cause-legend-val" style="position:absolute;display:inline-block;left:260px;top:1067px;font-size:20px;color: #8da3c4;font-weight: bold;"><%=totalPhlegmCnt %>번</div>
				...
				<div class="cause-legend-val" style="position:absolute;display:inline-block;left:407px;top:1067px;font-size:20px;color: #bec7d0;font-weight: bold;"><%=totalChestCnt %>번</div>
				...
				<div class="cause-legend-val" style="position:absolute;display:inline-block;left:566px;top:1067px;font-size:20px;color: #f07a55;font-weight: bold;"><%=totalBreathCnt %>번</div>
				...
				<div class="cause-legend-val" style="position:absolute;display:inline-block;left:699px;top:1067px;font-size:20px;color: #f7c24f;font-weight: bold;"><%=totalRoaringCnt %>번</div>
				...
				<div class="cause-legend-val" style="position:absolute;display:inline-block;left:846px;top:1067px;font-size:20px;color: #1388ac;font-weight: bold;"><%=totalEtcCnt %>번</div>
				...
				<div class="cause-legend-val" style="position:absolute;display:inline-block;left:993px;top:1067px;font-size:20px;color: #8d8d8d;font-weight: bold;"><%=totalNoSymptomCnt %>번</div>
				
				<div class="cause-history-chart-unit" style="position:absolute;display:inline-block;left:993px;top:1101px; font-weight: bold;font-size:12px;color: #000000;">단위(번)</div>
		...
		function loadChart() {
			...
			var mobileHistoryBarChartOptions = {
					...
					options: {
						...
						scales: { xAxes: [{barPercentage: 0.1, display: false,stacked: true, ticks:{max:<%=arrCauseSize%>}}],yAxes: [{display: false,stacked: true }]},
						legend:{ display:false},
			...
			for(int i=0;i<mediNoArr.size();i++){
				...
				int loss = (int)(mediGoalCount.get(i)*mediFrqArr.get(i)-0)  - (emgMedi+norMedi);
				if(loss<0){
					loss = 0;
				}
				String piBuff = "";
